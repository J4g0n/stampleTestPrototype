package objectLocatorRepository

import objectLocatorRepository.objectMapUtils.{Selector, ObjectMapRepository}

/**
  * Created by dev on 15/12/15.
  */
trait MaximisedStampleComponentSelectors extends ObjectMapRepository {
  val MAXIMISED_STAMPLE: Selector = "maximisedStample.stample"
  val MAXIMISED_STAMPLE_CLOSE_BUTTON: Selector = "maximisedStample.closeButton"
  val MAXIMISED_STAMPLE_TITLE: Selector = "maximisedStample.title"
  val MAXIMISED_STAMPLE_SUMMARY: Selector = "maximisedStample.summary"
  val MAXIMISED_STAMPLE_DESCRIPTION: Selector = "maximisedStample.description"
  val MAXIMISED_STAMPLE_COMMENT: Selector = "maximisedStample.comment"
  val MAXIMISED_STAMPLE_COMMENT_CONTENT: Selector = "maximisedStample.commentContent"
  val MAXIMISED_STAMPLE_CREATOR: Selector = "maximisedStample.userCreator"
  val MAXIMISED_STAMPLE_USER_LAST_EVENT: Selector = "maximisedStample.userLastEvent"
  val MAXIMISED_STAMPLE_CATEGORY: Selector = "maximisedStample.category"
  val MAXIMISED_STAMPLE_DESCRIPTION_PICTURE: Selector = "maximisedStample.description.picture"
  val MAXIMISED_STAMPLE_DESCRIPTION_VIDEO: Selector = "maximisedStample.description.video"
  val MAXIMISED_STAMPLE_FILE_ATTACHMENT_NAME: Selector = "maximisedStample.fileAttachmentName"
  val MAXIMISED_STAMPLE_REMINDER_BUTTON: Selector = "maximisedStample.reminderButton"
  val MAXIMISED_STAMPLE_FAVORITE_BUTTON: Selector = "maximisedStample.favoriteButton"
  val MAXIMISED_STAMPLE_COMMENT_BUTTON: Selector = "maximisedStample.commentButton"
  val MAXIMISED_STAMPLE_TAG_BUTTON: Selector = "maximisedStample.tagButton"
  val MAXIMISED_STAMPLE_MORE_BUTTON: Selector = "maximisedStample.moreButton"
  val MAXIMISED_STAMPLE_LIKE_BUTTON: Selector = "maximisedStample.likeButton"
  val MAXIMISED_STAMPLE_REMINDER_SET_BUTTON: Selector = "maximisedStample.reminderSet"
  val MAXIMISED_STAMPLE_HASHTAG_EDITOR: Selector = "maximisedStample.hashtagEditor"
  val MAXIMISED_STAMPLE_HASHTAG_TEXT: Selector = "maximisedStample.tagText"
  val MAXIMISED_STAMPLE_HASHTAG_DELETE_BUTTON: Selector = "maximisedStample.tagDeleteButton"
  val MAXIMISED_STAMPLE_NEW_COMMENT_EDITOR: Selector = "maximisedStample.newCommentEditor"
  val MAXIMISED_STAMPLE_NEW_COMMENT_CANCEL_BUTTON: Selector = "maximisedStample.newCommentCancelButton"
  val MAXIMISED_STAMPLE_NEW_COMMENT_SAVE_BUTTON: Selector = "maximisedStample.newCommentSaveButton"
  val MAXIMISED_STAMPLE_COMMENT_MORE_BUTTON: Selector = "maximisedStample.commentMoreButton"
  val MAXIMISED_STAMPLE_COMMENT_EDITOR: Selector = "maximisedStample.commentEditor"
  val MAXIMISED_STAMPLE_COMMENT_EDITION_CANCEL_BUTTON: Selector = "maximisedStample.commentEditionCancelButton"
  val MAXIMISED_STAMPLE_COMMENT_EDITION_SAVE_BUTTON: Selector = "maximisedStample.commentEditionSaveButton"
  val MAXIMISED_STAMPLE_CONFIRM_DELETE_BUTTON: Selector = "maximisedStample.confirmDeleteComment"
  val MAXIMISED_STAMPLE_CANCEL_DELETE_BUTTON: Selector = "maximisedStample.cancelDeleteComment"
}
